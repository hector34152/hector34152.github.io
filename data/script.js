let menuC = [
  ...
  {name:"Recomendación del Mes",label:"recomendacion_mes"},
  {name:"Recomendación del Mes",label:"recomendacion_mes"}
]
let zac = [
...
<header><img src="./img/logoGarufa.png"></header><main id="main"></main>`;
    ...
    for(let i=0;i<menuC.length;i++){
        document.getElementById("main").innerHTML += `<div `+(i==menuC.length-1 ? "id = 'mes' onclick = recomendado("+i+")": "onclick = categoria("+i+")")+`>`+menuC[i].name+`</div>`;
    }
...
function recomendado(a){
  document.getElementById("body").innerHTML = 
  `<link rel="stylesheet" href="./styles/mainReco.css"><a id= "volver" onclick="`+(a==0 ? "categoria(0)" : "crear()")+`"><img src="img/volver.png"></a>
  <style> main{grid-template-columns: repeat(`+(a==0 ? botella[a][1].length : botella[a].length)+`,100%)}</style>
  <header><p><img src="./img/logoGarufa.png"><stronge><img src="./img/flags/0`+( a > 7 ? 7 : a )+`.png"><br>`+(a==0 ? zac[1].name : menuC[a].name )+`</stronge></p></header>
  <main id ="main">`;
  for(let i=0;i<(a==0 ?botella[a][1].length:botella[a].length);i++){
    if((a==0 ?botella[a][1][i].flag:botella[a][i].flag)==true){
      document.getElementById("main").innerHTML += `
      <div class = "card"><p onclick="info(`+a+`,`+1+`,`+i+`)"><img src='./img/`+menuC[a].label+(a==0 ? "/"+zac[1].label+`/0`+(i+1) : "/0"+(i+1))+`.png'></p>
      <div class = "info">
        <h2 class = "h`+i+` active" onclick="this.classList.add('remove');this.classList.remove('active');document.querySelector('.lista`+i+`').classList.remove('remove')">`+(a==0 ?botella[a][1][i].nombre:botella[a][i].nombre)+`</h2>
      <div class = "lista`+i+` remove" onclick = "this.classList.add('remove');this.classList.remove('active');document.querySelector('.h`+i+`').classList.add('active')">
        <a class = "precio"><img class="icon" src="./img/botella.png"><p>(750ml) <stronge>`+(a==0 ?botella[a][1][i].precioA:botella[a][i].precioA)+`<stronge><br></p></a>
        `+((a==0 ?botella[a][1][i].precioB:botella[a][i].precioB)!="no" ? `<a class = "precio"><img class="icon" src="./img/copa.png"><p>(375ml)  <stronge>`+(a==0 ?botella[a][1][i].precioB:botella[a][i].precioB)+`<stronge></p></a>`: "")+`
      </div>
      </div>`;
    }
  }
